...
//$f3rs = ResultObject::find_all();


$sql  = "SELECT * FROM  firstsemester3 ";
$sql .= "LIMIT {$per_page} ";
$sql .= "OFFSET {$pagination->offset()}";
$f3rs = ResultObject::find_by_sql($sql);
...
<table class="table table-striped table-bordered table-hover table-condensed text-center">
	...
	<?php foreach($f3rs as $f3r) {?>
		<tr class="text-info">
			<td><?php  echo h($f3r->id) ;?></td>
			<td><?php echo h($f3r->matnumber);?></td>
            <td><a class="action" href="<?php echo url_for('/admin/upload/stageone.php') ;?>">click to reupload</a></td>
			<td><a class="action" href="<?php echo url_for('/admin/result/f3/view_result.php?id=' . $f3r->id) ;?>">click to view</a></td>
			
			
		</tr>
		<?php }?>
		<?php 
    	$url=url_for('/admin/result/f3/index.php');
